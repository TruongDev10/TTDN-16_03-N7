...
class ProjectTask(models.Model):
    ...
    def write(self, vals):
        """Override write to check permission before done"""
        if 'state' in vals and vals['state'] == 'done':
            for record in self:
                # Check permission: Only Project Manager of the project or Admin can mark as done
                if not self.env.user.has_group('base.group_system') and record.project_id.user_id != self.env.user:
                    raise models.ValidationError("Chỉ 'Người quản lý' dự án mới có quyền đánh dấu Hoàn thành công việc này.")
